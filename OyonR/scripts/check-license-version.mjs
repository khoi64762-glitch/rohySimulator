/**
 * Report whether the canonical Carm License has moved past the version Oyon
 ...
 *
 *   node scripts/check-license-version.mjs
 */
...
try {
  ...
  console.warn(`Oyon pins v${CARM_LICENSE_VERSION}; currency not checked.`);
  process.exit(0);
}
...
if (current === null) {
  ...
  console.warn(`Oyon pins v${CARM_LICENSE_VERSION}.`);
  process.exit(0);
}
...
console.log(
  `Carm Research License v${current} is now current; Oyon pins v${CARM_LICENSE_VERSION}.\n` +
  '\nTo adopt it:\n' +
  '  1. set CARM_LICENSE_VERSION in scripts/licenses.manifest.mjs\n' +
  '  2. npm run license:sync            (rewrites LICENSE from the new tag)\n' +
  '  3. update the version string in README.md, NOTICE.md, the About page and\n' +
  '     the tests that assert it (npm run license:verify lists them)\n' +
