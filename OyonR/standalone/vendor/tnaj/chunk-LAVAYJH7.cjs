...
function qt(p, df) {
  ...
  for (let i = 0; i < 200; i++) {
    const mid = (lo + hi) / 2;
    const cdf = mid >= 0 ? 1 - chunkO5U3BFYT_cjs.pt2(mid, df) / 2 : chunkO5U3BFYT_cjs.pt2(mid, df) / 2;
    if (cdf < p) lo = mid;
    else hi = mid;
...
function welchAnova(groups) {
  ...
  return { F, df1: k - 1, df2, p: chunkO5U3BFYT_cjs.pf(F, k - 1, df2) };
}
...
function welchPairs(names, groups, adjust = "holm") {
  const raw = pairsOf(groups).map(([i, j]) => {
    ...
    return {
      ...
      statistic: t,
      df,
      p: chunkO5U3BFYT_cjs.pt2(t, df),
      ciLower: m2 - m1 - tc * se,
      ciUpper: m2 - m1 + tc * se,
      effect: hedgesG(y, x)
    };
  });
  const adj = Array.from(chunkZKTLZLRH_cjs.pAdjust(raw.map((r) => r.p), adjust));
...
function leveneBF(groups) {
  ...
  const a = chunkO5U3BFYT_cjs.anova(z);
...
function analyseCovariate(covariate, names, groups, options) {
  ...
  const a = chunkO5U3BFYT_cjs.anova(gs);
  const w = welchAnova(gs);
  const kw = chunkO5U3BFYT_cjs.kruskal(gs);
...
exports.analyseCovariate = analyseCovariate;
exports.describeGroups = describeGroups;
exports.dunnTest = dunnTest;
exports.effectSizes = effectSizes;
exports.gamesHowell = gamesHowell;
exports.hedgesG = hedgesG;
exports.leveneBF = leveneBF;
exports.pnorm = pnorm;
exports.postHoc = postHoc;
exports.prange = prange;
exports.ptukey = ptukey;
exports.ptukeyUpper = ptukeyUpper;
exports.qtukey = qtukey;
exports.quantile7 = quantile7;
exports.tukeyHSD = tukeyHSD;
exports.welchAnova = welchAnova;
exports.welchPairs = welchPairs;
//# sourceMappingURL=chunk-LAVAYJH7.cjs.map
//# sourceMappingURL=chunk-LAVAYJH7.cjs.map