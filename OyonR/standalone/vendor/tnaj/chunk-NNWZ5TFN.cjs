...
var chunkGF6HGQDU_cjs = require('./chunk-GF6HGQDU.cjs');
...
function htna(data, options) {
  ...
  const model = chunkRT5XI5AH_cjs.buildModel(data, buildOpts);
...
function matrixView(value, name) {
  if (value instanceof chunkRT5XI5AH_cjs.Matrix) {
...
function prepareTna(model, options) {
  ...
  const macro = new chunkRT5XI5AH_cjs.Matrix(names.length, names.length);
  ...
  for (let c = 0; c < names.length; c++) {
    const name = names[c];
    const idx = indices[c];
    const matrix = new chunkRT5XI5AH_cjs.Matrix(idx.length, idx.length);
...
function prepareExisting(input, options) {
  ...
  for (const name of names) {
    ...
    if (!layer) {
      clusters[name] = { labels, weights: new chunkRT5XI5AH_cjs.Matrix(labels.length, labels.length), inits: new Array(labels.length).fill(0) };
      continue;
    }
...
function plotMCML(input, options = {}) {
  ...
  const renderSummaryEdge = (edge) => {
    ...
    const geometry = edge.from === edge.to ? chunkGF6HGQDU_cjs.networkSelfLoopGeometry(
      from.x,
      from.y,
      paintedRadius,
      Math.atan2(from.y - summaryCenterY, from.x - summaryCenterX)
    ) : chunkGF6HGQDU_cjs.networkEdgeGeometry(
      from.x,
      from.y,
      to.x,
      to.y,
    ...
    return `<path class="mcml-edge mcml-edge-summary" data-from="${esc(edge.from)}" data-to="${esc(edge.to)}" data-weight="${edge.weight}" d="${geometry.path}" fill="none" stroke="${esc(edge.color)}" stroke-width="${edge.width}" stroke-opacity="${edge.opacity}" stroke-linecap="round"/>` + (edge.directed ? `<polygon class="mcml-summary-arrow" points="${chunkGF6HGQDU_cjs.networkArrowPolygon(geometry.tipX, geometry.tipY, geometry.tipDx, geometry.tipDy)}" fill="${esc(edge.color)}" fill-opacity="${Math.min(1, edge.opacity + 0.18)}"/>` : "") + (edge.label ? `<text class="mcml-edge-label" x="${geometry.labelX}" y="${labelY}" text-anchor="middle">${esc(edge.label)}</text>` : "");
  };
...
exports.HTNA_COLOR_PALETTE = HTNA_COLOR_PALETTE;
exports.HTNA_SHAPE_PALETTE = HTNA_SHAPE_PALETTE;
exports.MCML_COLOR_PALETTE = MCML_COLOR_PALETTE;
exports.bootstrapHtna = bootstrapHtna;
exports.diffNetworks = diffNetworks;
exports.extractMetaPaths = extractMetaPaths;
exports.formatPaths = formatPaths;
exports.htna = htna;
exports.htnaFromLong = htnaFromLong;
exports.htnaPlotData = htnaPlotData;
exports.mcmlPlotData = mcmlPlotData;
exports.mcml_plot_data = mcml_plot_data;
exports.plotMCML = plotMCML;
exports.plot_mcml = plot_mcml;
exports.sequencePlotHtnaData = sequencePlotHtnaData;
//# sourceMappingURL=chunk-NNWZ5TFN.cjs.map
//# sourceMappingURL=chunk-NNWZ5TFN.cjs.map