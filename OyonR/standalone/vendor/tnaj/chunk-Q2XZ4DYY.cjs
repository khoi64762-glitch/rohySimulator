...
function bootstrapTna(model, options = {}) {
  ...
  const weights = chunkRT5XI5AH_cjs.computeWeightsFrom3D(trans, modelType, modelScaling);
  ...
  for (let it = 0; it < iter; it++) {
    ...
    const wBoot = chunkRT5XI5AH_cjs.computeWeightsFrom3D(transBoot, modelType, modelScaling);
  ...
  for (let idx = 0; idx < a * a; idx++) {
    ciLower[idx] = chunkRT5XI5AH_cjs.arrayQuantile(bootWeights[idx], halfLevel);
    ciUpper[idx] = chunkRT5XI5AH_cjs.arrayQuantile(bootWeights[idx], 1 - halfLevel);
  }
  ...
  const sigModel = chunkRT5XI5AH_cjs.createTNA(weights, model.inits, labels, model.data, model.type, model.scaling);
  for (let i = 0; i < a; i++) {
    for (let j = 0; j < a; j++) {
      const idx = i * a + j;
...
function bootstrapWtna(input, options = {}) {
  ...
  for (let it = 0; it < iter; it++) {
    ...
    const tempModel = chunkRT5XI5AH_cjs.buildModel(matrix, tempOpts);
    for (let i = 0; i < a; i++) {
      for (let j = 0; j < a; j++) {
        const idx = i * a + j;
  ...
  const halfLevel = level / 2;
  for (let idx = 0; idx < a * a; idx++) {
    ciLower[idx] = chunkRT5XI5AH_cjs.arrayQuantile(bootWeightsArr[idx], halfLevel);
    ciUpper[idx] = chunkRT5XI5AH_cjs.arrayQuantile(bootWeightsArr[idx], 1 - halfLevel);
  }
  const weightsMean = new Float64Array(a * a);
  const weightsSd = new Float64Array(a * a);
  for (let i = 0; i < a * a; i++) {
  ...
  const sigModel = chunkRT5XI5AH_cjs.buildModel(sigWeights2D, { type: originalModel.type, labels });
  return { edges, model: sigModel, labels, method, iter, level, weightsMean, weightsSd, weightsBias };
}

exports.applyIntervalWindowing = applyIntervalWindowing;
exports.applyWindowing = applyWindowing;
exports.bootstrapTna = bootstrapTna;
exports.bootstrapWtna = bootstrapWtna;
exports.buildWtnaMatrix = buildWtnaMatrix;
exports.computeWithinWindow = computeWithinWindow;
exports.computeWtnaTransitions = computeWtnaTransitions;
exports.rowNormalizeWtna = rowNormalizeWtna;
exports.toBinaryMatrix = toBinaryMatrix;
//# sourceMappingURL=chunk-Q2XZ4DYY.cjs.map
//# sourceMappingURL=chunk-Q2XZ4DYY.cjs.map