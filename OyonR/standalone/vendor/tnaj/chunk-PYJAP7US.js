...
function chiSquareCritical(alpha, degreesOfFreedom) {
  ...
  while (pchisq(high, degreesOfFreedom) > alpha) high *= 2;
  for (let iteration = 0; iteration < 100; iteration++) {
    const midpoint = (low + high) / 2;
    if (pchisq(midpoint, degreesOfFreedom) > alpha) low = midpoint;
    else high = midpoint;
  }
  return (low + high) / 2;
}
...
function tuneTree(data, options = {}) {
  ...
  const order = new SeededRNG(options.seed ?? 1).permutation(data.length);
...
//# sourceMappingURL=chunk-PYJAP7US.js.map
//# sourceMappingURL=chunk-PYJAP7US.js.map