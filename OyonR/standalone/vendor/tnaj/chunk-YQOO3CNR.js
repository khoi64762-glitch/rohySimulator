import { pchisq } from './chunk-DOMOOFBU.js';
import { SeededRNG } from './chunk-2EXGRQR6.js';
...
var rngFor = (seed) => new SeededRNG(seed ?? Date.now() & 4294967295);
...
function bayesTwo(a, b, names, options) {
  const prior = options.prior ?? 0.5, draws = options.draws ?? 1e4, ci = options.confidenceLevel ?? 0.95, meanThreshold = options.meanThreshold ?? 0.01, boundThreshold = options.boundThreshold ?? 1e-3, rng = new SeededRNG(options.seed ?? Date.now() & 4294967295), tail = (1 - ci) / 2, rawP = [], temp = [];
...
function transferEntropy(x, y, options = {}) {
  ...
  const rng = new SeededRNG(options.seed ?? Date.now() & 4294967295), xs = sequences(x), opts = { lag, history, iterations, test }, out = [];
  if (y) {
...
export { bayesCompareLSA, bootstrapLSA, certaintyLSA, compareLSA, getLSAEngine, initial, lagProfile, listLSAEngines, lsa, lsaBidirectional, lsaClassical, lsaData, lsaDataFromMatrix, lsaDataFromSequences, lsaIPF, lsaLags, lsaNonparallelDominance, lsaParallelDominance, lsaToTNA, lsaTransitions, lsaTwoCell, nodes, permuteLSA, plotChords, plotComparison, plotForest, plotLSA, plotLSAHeatmap, plotPolar, plotTransitions, prepareLong, registerLSAEngine, reliabilityLSA, stabilityLSA, tests, transferEntropy, transitionProbabilities, transitions, unregisterLSAEngine };
//# sourceMappingURL=chunk-YQOO3CNR.js.map
//# sourceMappingURL=chunk-YQOO3CNR.js.map