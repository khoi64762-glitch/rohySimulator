...
Once you've installed (see [Install](/operator/install)), this document
...
see [Updating](/operator/updating).

::: tip Reference
The full env-var catalogue is single-sourced in
[Config & env](/reference/config/); CLI flags and exit codes in
[CLI & ops](/reference/cli/). Terms are locked in the
...
- [ ] **Retention sweep cron installed** (see the [Retention & purges runbook](/operator/retention)).
- [ ] **Off-site backup configured** (see [Updating § Off-site backups](/operator/updating#off-site-backups)).
- [ ] **Observability log shipper wired** to NDJSON stdout, or set `ROHY_LOG_LEVEL=warn` if you don't ship logs (see [Observability runbook](/operator/observability)).
...
Set in `/etc/rohy/env` (systemd) or `deploy/docker/.env` (compose). The
authoritative list is [Config & env](/reference/config/); the
production-critical subset is below.
...
Replace `<password>` with the real password of a dedicated verifier
account. The credentials must belong to a real rohy user — any role
works, the contract probe doesn't read data, it just needs to pass
...
Retention**. Per-tenant Oyon retention is set in **Settings → Oyon**
(uses `oyon_settings.retention_days`). Full detail is in the
[Retention & purges runbook](/operator/retention).
...
default. See [Updating § Off-site backups](/operator/updating#off-site-backups)
for rsync and rclone recipes (1-line cron each). Full drill in the
[Backup & restore runbook](/operator/backup-restore).
...
Replace `<snapshot-name>` with a name from `list-backups`.
...
4. Re-stamp baseline + dump from SQLite to Postgres if you have data to migrate
...
demotions / suspensions take effect immediately. The educator role is
surfaced in the UI as **Teacher**.
...
users via `users.tenant_id`. The admin UI exposes this under
**Settings → Users → Tenant assignment**.

### Cohorts

Optional grouping within a tenant. `cohorts` table + `cohort_members`.
Used by analytics roll-ups to filter by class / group / cohort. See the
[Glossary](/reference/glossary) for the cohort / class terminology lock.
...
- **Need to upgrade?** — [Updating](/operator/updating)
- **Audit / compliance review?** — `docs/audits/` in the repo
