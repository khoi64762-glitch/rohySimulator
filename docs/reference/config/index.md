...
| `ROHY_SHUTDOWN_GRACE_MS` | No | — | Graceful-shutdown drain window (ms). | `server/server.js:367` |
...
| Variable | Required | Default | Purpose | Source |
| --- | --- | --- | --- | --- |
| `OYON_ENABLED` | No | — | Mounts the Oyon emotion-capture addon as a live router (vs 503 stub). | `server/routes.js:37`<br>`server/routes/help-routes.js:129` |
...
_47 variables discovered. Generated by `scripts/docs-gen/gen-config.mjs` — regenerate with `npm run docs:gen:config`._
